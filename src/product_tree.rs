...
impl<F: PrimeField> ProductCircuitEvalProof<F> {
  #![allow(dead_code)]
  pub fn prove(circuit: &mut ProductCircuit<F>, transcript: &mut Transcript) -> (Self, F, Vec<F>) {
  ...
  pub fn verify(&self, eval: F, len: usize, transcript: &mut Transcript) -> (F, Vec<F>) {
